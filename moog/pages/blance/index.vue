<template>
	<view class="blacnceContain">
		<cu-custom bgColor="bg-black" :isBack="true">
			...
			<block slot="right"><span class="iconfont icon-kefu" @tap="navigateTo('/pages/customerSerice/index')"></span></block>
		</cu-custom>
		...
		<view class="swiper-area">
			<swiper :acceleration="false" :current="currentTabs" @change="change">
				<swiper-item v-for="(item, index) in tabs" :key="index">
					<scroll-view scroll-y :style="{ height: scrollViewHeight + 'px', background: '#f5f5f5' }" :refresher-threshold="100" refresher-background="#f5f5f5">
						<view style="height:100%" v-if="true"><pay @commit="commit" :card="item.card" :payRadio="item.channel_list"></pay></view>
					</scroll-view>
				</swiper-item>
			</swiper>
		</view>
